...
public:
	...
	T &operator[](int i)
	{
		Assert(IsIdxValid(i)); return Base()[i];
	}
	...
	{
		int limit = NumAllocated(); return Iterator_t(limit ? 0 : InvalidIndex(), limit);
	}
...
DECLARE_DEDUCE_FIELDTYPE( FIELD_EHANDLE,	void * );
...
DECLARE_NAMED_FIELDTYPE( void *, "ehandle" );
...
public:
	...
	virtual bool ForwardConsoleCommand( const CCommandContext &commandContext, const CCommand &command ) = 0;
	...
	virtual HSCRIPT CreateArray( ScriptVariant_t &Array ) = 0;
	virtual bool IsArray( HSCRIPT hScope ) = 0;
	virtual int GetArrayCount( HSCRIPT hScope ) = 0;
	virtual void ArrayAddToTail( HSCRIPT hScope, const ScriptVariant_t &value ) = 0;

	//----------------------------------------------------------------------------

	virtual void WriteState( CUtlBuffer *pBuffer ) = 0;
	virtual void ReadState( CUtlBuffer *pBuffer ) = 0;
	virtual void CollectGarbage( const char *szUnknown, bool bUnknown ) = 0;
	...
	virtual HSCRIPT CopyHandle( HSCRIPT hScript ) = 0;
	virtual int GetIdentity( HSCRIPT hScript ) = 0;

	class ISquirrelMetamethodDelegate;

	virtual void *MakeSquirrelMetamethod_Get( HSCRIPT &hScript, const char *szUnknown, ISquirrelMetamethodDelegate *pSquirrelMetamethodDelegate, bool bUnknown ) = 0;

	virtual void DestroySquirrelMetamethod_Get( CSquirrelMetamethodDelegateImpl *pSquirrelMetamethodDelegateImpl ) = 0;
